<script>
export default {
    setup() {
        function getImageUrl(name) {
            return new URL(`/src/assets/${name}`, import.meta.url).href
        }

        return {
            getImageUrl,
        }
    }
}
</script>

<template>
    <div class="w-full bg-zinc-100 py-10 pb-16" id="fitur">
        <div class="md:max-w-[1024px] mx-auto">
            <h1 class="text-4xl mb-10 text-center">Fitur</h1>
            <div class="grid grid-cols-3 gap-10">
                <div class="text-center">
                    <div class="logoftr">
                        <svg viewBox="0 0 64 64">
                            <g>
                                <path
                                    d="M46,22h-6.671l5.557-6.352C45.604,14.828,46,13.775,46,12.685V12.5c0-2.481-2.019-4.5-4.5-4.5S37,10.019,37,12.5V14h2v-1.5   c0-1.379,1.121-2.5,2.5-2.5s2.5,1.121,2.5,2.5v0.185c0,0.606-0.22,1.19-0.619,1.646L37,21.624V24h9V22z" />
                                <path
                                    d="M54,24h2v-3h2v-2h-2V8h-2.618L48,18.764V21h6V24z M50.118,19L54,11.236V19H50.118z" />
                                <path
                                    d="M50.405,32.627C57.596,31.061,63,24.653,63,17c0-8.822-7.178-16-16-16c-7.653,0-14.061,5.404-15.627,12.595   C29.608,13.207,27.807,13,26,13C12.215,13,1,24.215,1,38s11.215,25,25,25s25-11.215,25-25C51,36.193,50.793,34.392,50.405,32.627z    M47,3c7.72,0,14,6.28,14,14s-6.28,14-14,14s-14-6.28-14-14S39.28,3,47,3z M44.961,37H43v2h1.949   c-0.235,4.484-2.024,8.553-4.847,11.688l-1.375-1.375l-1.414,1.414l1.375,1.375c-3.135,2.824-7.204,4.612-11.688,4.847V55h-2v1.949   c-4.484-0.235-8.553-2.024-11.688-4.847l1.375-1.375l-1.414-1.414l-1.375,1.375C9.074,47.553,7.286,43.484,7.051,39H9v-2H7.051   c0.235-4.484,2.024-8.553,4.847-11.688l1.375,1.375l1.414-1.414l-1.375-1.375c3.135-2.824,7.204-4.612,11.688-4.847V21h2v-1.961   c1.438,0.076,2.862,0.318,4.255,0.72c1.156,6.611,6.374,11.829,12.986,12.986C44.642,34.138,44.884,35.562,44.961,37z M26,61   C13.317,61,3,50.683,3,38s10.317-23,23-23c1.706,0,3.407,0.201,5.072,0.577C31.03,16.047,31,16.52,31,17   c0,0.209,0.023,0.413,0.031,0.621C29.381,17.212,27.695,17,26,17C14.421,17,5,26.421,5,38s9.421,21,21,21s21-9.421,21-21   c0-1.695-0.212-3.381-0.621-5.031C46.587,32.977,46.791,33,47,33c0.48,0,0.953-0.03,1.423-0.072C48.799,34.593,49,36.294,49,38   C49,50.683,38.683,61,26,61z" />
                                <path
                                    d="M29,38c0-1.302-0.839-2.402-2-2.816V23h-2v12.184c-1.161,0.414-2,1.514-2,2.816c0,1.654,1.346,3,3,3   c0.462,0,0.894-0.113,1.285-0.301l7.008,7.008l1.414-1.414l-7.008-7.008C28.887,38.894,29,38.462,29,38z M25,38   c0-0.552,0.448-1,1-1s1,0.448,1,1s-0.448,1-1,1S25,38.552,25,38z" />
                                <path
                                    d="M56,35c-1.654,0-3,1.346-3,3c0,1.251,0.771,2.324,1.862,2.773c-0.627,6.584-3.499,12.781-8.152,17.527l1.428,1.4   c4.999-5.098,8.074-11.763,8.725-18.842C58.095,40.485,59,39.353,59,38C59,36.346,57.654,35,56,35z M56,39c-0.552,0-1-0.448-1-1   s0.448-1,1-1s1,0.448,1,1S56.552,39,56,39z" />
                                <path
                                    d="M6.283,16.733c4.658-4.321,10.651-6.978,16.949-7.579C23.686,10.237,24.755,11,26,11c1.654,0,3-1.346,3-3s-1.346-3-3-3   c-1.352,0-2.485,0.905-2.858,2.137c-6.771,0.624-13.214,3.489-18.218,8.13L6.283,16.733z M26,7c0.552,0,1,0.448,1,1s-0.448,1-1,1   s-1-0.448-1-1S25.448,7,26,7z" />
                            </g>
                        </svg>
                    </div>
                    <h2 class="text-xl mb-2">Free Support</h2>
                    <p class="text-sm">Kami selalu memberikan support yang baik serta pelayanan yang ramah kepada
                        pelanggan</p>
                </div>
                <div class="text-center">
                    <div class="logoftr">
                        <svg viewBox="0 0 200 200">
                            <defs></defs>
                            <title>Wondicon - UI (Free)</title>
                            <path class="cls-1"
                                d="M118.75,157a4.87,4.87,0,0,1-.66,0,5,5,0,0,1-3.67-2.46l-11.73-20.32L91,132.41A27.58,27.58,0,0,1,67.59,109l-1.73-11.7L45.54,85.58a5,5,0,0,1-2.46-3.67,5,5,0,0,1,1.42-4.19L62.18,60a5,5,0,0,1,3.53-1.46H99l24.76-24.77a32.31,32.31,0,0,1,26-9.35l19.46,1.83a5,5,0,0,1,4.51,4.51l1.83,19.46a32.3,32.3,0,0,1-9.35,26L141.42,101v33.29a5,5,0,0,1-1.46,3.53L122.28,155.5A5,5,0,0,1,118.75,157ZM56.18,80.18,73,89.86a5,5,0,0,1,2.45,3.6l2.08,14.09a17.65,17.65,0,0,0,15,15l14.09,2.08a5,5,0,0,1,3.6,2.45l9.68,16.77,11.6-11.61V98.93a5,5,0,0,1,1.47-3.53l26.23-26.23a22.39,22.39,0,0,0,6.47-18l-1.45-15.34L148.8,34.41a22.41,22.41,0,0,0-18,6.47L104.6,67.11a5,5,0,0,1-3.53,1.47H67.79Z" />
                            <path class="cls-1"
                                d="M29.31,175.69a5,5,0,0,1-3.53-8.54l35.33-35.33a5,5,0,1,1,7.07,7.07L32.85,174.22A5,5,0,0,1,29.31,175.69Z" />
                            <path class="cls-1"
                                d="M28.91,135.13a5,5,0,0,1-3.54-8.53l21.09-21.1a5,5,0,1,1,7.08,7.07l-21.1,21.1A5,5,0,0,1,28.91,135.13Z" />
                            <path class="cls-1"
                                d="M69.46,175.69a5,5,0,0,1-3.53-8.54L87,146.06a5,5,0,1,1,7.07,7.07L73,174.22A5,5,0,0,1,69.46,175.69Z" />
                            <path class="cls-1"
                                d="M141,73a14,14,0,1,1,14-14A14,14,0,0,1,141,73Zm0-18a4,4,0,1,0,4,4A4,4,0,0,0,141,55Z" />
                        </svg>
                    </div>
                    <h2 class="text-xl mb-2">Internet Up To</h2>
                    <p class="text-sm">Tanpa Kuota internet sepuasnya</p>
                </div>
                <div class="text-center">
                    <div class="logoftr">
                        <svg viewBox="0 0 64 64">
                            <g>
                                <path
                                    d="M60,35c-0.395,0-0.77,0.081-1.116,0.22C58.539,33.945,57.383,33,56,33c-0.395,0-0.77,0.081-1.116,0.22   C54.539,31.945,53.383,31,52,31c-0.352,0-0.686,0.072-1,0.184v-1.543C56.841,28.269,61,23.088,61,17   c0-2.932-0.987-5.631-2.631-7.808l2.359-2.507l-1.457-1.371l-2.227,2.367C54.702,5.407,51.514,4,48,4c-7.168,0-13,5.832-13,13   c0,6.088,4.159,11.269,10,12.642v17.937c-0.188-0.193-0.322-0.442-0.374-0.725l-1.317-7.241C43.034,38.099,41.719,37,40.181,37   C38.427,37,37,38.427,37,40.18v0.639c0,0.174,0.015,0.349,0.043,0.523l1.8,10.794c0.141,0.841,0.495,1.638,1.026,2.301l2.473,3.091   C42.767,58.06,43,58.725,43,59.403V63h16v-3.597c0-0.678,0.233-1.344,0.658-1.875l2.246-2.809C62.61,53.836,63,52.727,63,51.597V38   C63,36.346,61.654,35,60,35z M37,17c0-6.065,4.935-11,11-11c2.985,0,5.692,1.199,7.675,3.135L43.979,21.564l-2.271-2.271   l-1.414,1.414l3.729,3.729L56.98,10.668C58.248,12.461,59,14.642,59,17c0,4.99-3.302,9.262-8,10.58V26c0-1.654-1.346-3-3-3   s-3,1.346-3,3v1.58C40.302,26.262,37,21.99,37,17z M61,51.597c0,0.678-0.233,1.344-0.658,1.875l-2.246,2.809   C57.39,57.164,57,58.273,57,59.403V61H45v-1.597c0-1.13-0.39-2.24-1.096-3.124l-2.474-3.091c-0.318-0.398-0.531-0.875-0.615-1.381   l-1.8-10.792C39.005,40.951,39,40.885,39,40.819V40.18c0-0.651,0.529-1.18,1.181-1.18c0.57,0,1.059,0.408,1.161,0.97l1.316,7.241   C42.952,48.827,44.357,50,46,50h1V26c0-0.551,0.448-1,1-1s1,0.449,1,1v8v6v2h2v-2v-6c0-0.551,0.448-1,1-1s1,0.449,1,1v2v6h2v-6   c0-0.551,0.448-1,1-1s1,0.449,1,1v2v4h2v-4c0-0.551,0.448-1,1-1s1,0.449,1,1V51.597z" />
                                <path
                                    d="M26,1H8C4.141,1,1,4.14,1,8v48c0,3.86,3.141,7,7,7h18c3.859,0,7-3.14,7-7V8C33,4.14,29.859,1,26,1z M31,56   c0,2.757-2.243,5-5,5H8c-2.757,0-5-2.243-5-5V8c0-2.757,2.243-5,5-5h18c2.757,0,5,2.243,5,5V56z" />
                                <path
                                    d="M17,22.766l-3.132,6.256L6.829,30.03l5.095,4.895l-1.199,6.891L17,38.564l6.275,3.252l-1.199-6.891l5.095-4.895   l-7.039-1.008L17,22.766z M19.924,34.22l0.689,3.964L17,36.311l-3.613,1.873l0.689-3.964l-2.905-2.792l4.018-0.576L17,27.234   l1.812,3.619l4.018,0.576L19.924,34.22z" />
                                <path
                                    d="M17,41.783l-2.82,5.579l-6.365,0.903l4.607,4.386l-1.081,6.157L17,55.903l5.659,2.905l-1.081-6.157l4.607-4.386   l-6.365-0.903L17,41.783z M19.422,51.941l0.571,3.25L17,53.655l-2.993,1.537l0.571-3.25l-2.393-2.276l3.309-0.469L17,46.217   l1.506,2.979l3.309,0.469L19.422,51.941z" />
                                <path
                                    d="M11.341,21.808L17,18.903l5.659,2.905l-1.081-6.157l4.607-4.386l-6.365-0.903L17,4.783l-2.82,5.579l-6.365,0.903   l4.607,4.386L11.341,21.808z M12.186,12.666l3.309-0.469L17,9.217l1.506,2.979l3.309,0.469l-2.393,2.276l0.571,3.25L17,16.655   l-2.993,1.537l0.571-3.25L12.186,12.666z" />
                            </g>
                        </svg>
                        <h2 class="text-xl mb-2">Layanan</h2>
                        <p class="text-sm">Jasa Pelayananan dari warga untuk warga.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
<style scoped>
* {
    font-family: "TelkomselBatikSans-Bold", Helvetica, Arial;
}

p {
    font-family: Arial, Helvetica, sans-serif;
}

.logoftr svg {
    fill: #10a6a5;
    text-align: center;
    width: 30%;
    margin: 10px auto;
}
</style>